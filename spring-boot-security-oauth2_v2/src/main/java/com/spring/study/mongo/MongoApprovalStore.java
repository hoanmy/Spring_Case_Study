...
import com.spring.study.mongo.repositories.MongoApprovalRepository;
...
public class MongoApprovalStore implements ApprovalStore {
    ...
    private List<MongoApproval> transformToMongoApproval(final Collection<Approval> approvals) {
        return approvals.stream().map(approval -> new MongoApproval(UUID.randomUUID().toString(),
                ...
                com.spring.study.util.LocalDateTimeUtil.convertTolocalDateTimeFrom(approval.getExpiresAt()),
                com.spring.study.util.LocalDateTimeUtil.convertTolocalDateTimeFrom(approval.getLastUpdatedAt()))).collect(Collectors.toList());
    }
